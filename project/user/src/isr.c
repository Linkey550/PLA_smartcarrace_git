...
void UART7_IRQHandler (void)
{
    if(USART_GetITStatus(UART7, USART_IT_RXNE) != RESET)
    {
        ...
        extern void uart7_rx_interrupt_handler (void);
                uart7_rx_interrupt_handler();
...
//uint16_to_float函数
float* convert_uint16_to_float(uint16_t* input, size_t length) {
...
//int16_to_float函数
// 将 double 数组转换为 float 数组的函数
float* double_array_to_float_array(const double* double_array, int size) {
